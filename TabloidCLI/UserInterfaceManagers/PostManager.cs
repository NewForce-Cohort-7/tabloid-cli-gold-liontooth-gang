...
namespace TabloidCLI.UserInterfaceManagers
{
    public class PostManager : IUserInterfaceManager
    {
        ...
        private void Add()
        {
            ...
            post.Author = authorList[indexChoice - 1];

            List<Blog> blogList = _blogRepository.GetAll();
            for (int i = 0; i < blogList.Count; i++)
            {
                Console.WriteLine($"{i + 1}) {blogList[i].Title}");
            }
            Console.Write("> ");
            int blogIndexChoice = int.Parse(Console.ReadLine());
            post.Blog = blogList[blogIndexChoice - 1];
        ...
        private void Edit()
        {
            ...
            {
                postToEdit.PublishDateTime = Convert.ToDateTime(pubDate);
            }
